...
// 「結果を網羅済み」「1回戦免除」の断定は、この大会のresultsデータを実際に取り込んでいる
// 場合にのみ成立する。resultsが1日も無い大会(千葉2026の初期状態: 過去結果を取り込まない
// 運用)でこの断定をゲラに書くと、記事の正しいWeb調査由来の1回戦記述が全て「時制矛盾・
// 免除矛盾」として偽陽性になる(2026-07-10、千葉初回proofで13件の偽陽性が実発生)。
const hasResults = data.days.some((d) => d.kind === 'results');
...
for (const day of data.days) {
  ...
  for (const g of day.games) {
    ...
    const galley = [
      ...
      hasResults
        ? known
          ? `両校のこれまでの確定結果(今大会の全試合結果を網羅済み):\n${known}`
          : '(両校とも今大会はまだ試合をしていない)'
        : '(この大会の試合結果データはサイト側に未取り込み。記事中の1回戦結果・これまでの結果への言及はWeb調査由来で、このゲラの確定情報からは真偽を判定できない — 結果への言及を時制矛盾・1回戦免除との矛盾として指摘しないこと)',
      hasResults && byes.length
