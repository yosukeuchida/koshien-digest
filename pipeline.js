...
注意: 両校の過去の対戦成績を調べる際、この試合自体(${g.date}の${g.round})を「既に終わった過去の対戦」として結果付きで報告しないこと。似た名前の別の試合・別の年度・別の回戦・別の学校(「◯◯」と「県立◯◯」「◯◯学園」等の類似名)と混同しないよう、年度・大会名・正式な校名を必ず確認してから記載すること。
...
${sourceRule}
厳守: 事実は必ず検索結果に基づくこと。出典不明な情報は書かない。見つからなければ「見つからなかった」と正直に書く(小規模校では普通のこと)。各項目に出典URLを添える。`
}
